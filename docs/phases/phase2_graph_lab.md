...
- **2026-07-19 — đã chạy lại `run_etl.py` + `run_graph_builder.py` với pipeline mới (giữ thứ tự gốc).** Kết quả (`*_graphs.pt` mới, ghi đè bản cũ):
  - `nf-cse-cic-ids2018-v2`: tổng 3777 đồ thị con → train 2643, val 567, test 567.
  - `nf-unsw-nb15-v2`: tổng 477 đồ thị con → train 333, val 72, test 72.
  - Đã kiểm tra: đúng shape (10.000 cạnh/đồ thị, 39 đặc trưng/cạnh), đủ 15 lớp (CSE-CIC) và 10 lớp (UNSW-NB15) nhãn đa lớp xuất hiện. Số lượng đồ thị gần giống bản cũ (do cùng `WINDOW_SIZE`/`WINDOW_OVERLAP`) nhưng nội dung từng đồ thị giờ khác hẳn — dựng từ lát cắt thời gian thực thay vì mẫu ngẫu nhiên.
  - **Bước tiếp theo:** upload `data/processed/` mới lên Drive, train lại GCN/GAT trên Colab để so sánh với 2 lượt kết quả cũ (đã đánh dấu lỗi thời trong `docs/phases/phase3_model_training.md`).
