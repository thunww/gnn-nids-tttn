...
**🔗 Muốn viết báo cáo/lấy số liệu chính thức? Dùng thư mục `docs/graphsage/` (đã lọc sạch, không lẫn thông tin cũ).** File này chỉ là nhật ký làm việc.
...
**Đã điều tra kỹ, xác nhận đây KHÔNG PHẢI lỗi code:** đo trực tiếp cho thấy **69.2% số dòng (hàng) dữ liệu UNSW-NB15** có **ít nhất 1** trong 39 cột đặc trưng, sau khi quy đổi đúng cách sang thang đo của CSE-CIC, rơi vào vùng giá trị cực đoan (|z-score| > 5) — vùng model CSE-CIC chưa từng thấy lúc train (bình thường tỷ lệ này chỉ ~0.0001%). Hai môi trường mạng khác biệt tới mức việc "quy đổi đúng thang đo" vẫn đẩy hầu hết dữ liệu ra ngoài phạm vi model từng học được. *(Lưu ý phân biệt 2 cách đo, bổ sung 2026-07-27: 69,2% tính theo HÀNG (mẫu có ≥1 cột cực đoan); nếu tính theo từng GIÁ TRỊ đặc trưng đơn lẻ (% trên tổng số ô dữ liệu) thì tỷ lệ là 6,2% — xem biểu đồ `report_figures/zscore_distribution.png`, vẽ theo cách đo thứ 2.)*
